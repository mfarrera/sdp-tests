...
int load_vis(const char *filename, struct vis_data *vis,
    ...
    mem = sizeof(struct vis_data) + sizeof(struct bl_data)*vis->bl_count + stats.vis_count*sizeof(double complex) +
		stats.t_count*sizeof(double)*4 + stats.f_count*sizeof(double);
    return mem;
}
